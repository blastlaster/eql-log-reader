...
**Linux packages.** The suite now ships for Linux, two ways: a universal tarball (`eql-log-reader-1.8-linux.tar.gz` — unpack, `./install.sh`, per-user install with an `eql-log-reader` command and menu entry, no root) and a `.deb` for Debian/Ubuntu/Mint (`sudo apt install ./eql-log-reader_1.8_all.deb`, system-wide to `/opt` with per-user settings in `~/.local/share/eql-log-reader`). Both run on the distro's own `python3` + `python3-tk` — the suite is pure stdlib, so there's nothing else to install. Windows-only overlay niceties (chroma-key transparency/ghost mode) degrade to plain dark windows; everything else works, including reading a log synced from a Windows machine.
